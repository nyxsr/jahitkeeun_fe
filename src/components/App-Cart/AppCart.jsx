...
function AppCart() {
  ...
  const deleteCart = async (id)=>{
    const tanya = window.confirm('Apakah anda yakin ?')
    if (!tanya) {
      return;
    }
    try {
      const response = await axios.delete(`http://api.jahitkeeun.my.id/api/sectionitem/userId/${data.client.user_id}/service/${id}`,{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
      toast.success('Item Cart berhasil dihapus', {
        position: "top-center",
        autoClose: 5000,
  ...
  return (
    <AnimatePresence>
      <div className="h-screen bg-slate-200">
        <div className="w-screen md:w-[31.5rem] relative mx-auto py-10 pb-32 bg-[#FFF8EA] overflow-y-scroll h-screen">
          {isCheckout && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white/75 z-50 fixed md:w-[31.5rem] top-0 flex flex-col justify-center items-center w-full h-full"
            >
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="flex justify-end items-center gap-4">
                            <FaTrash className="text-xl text-red-500" onClick={()=>deleteCart(v.serviceId)} />
                        ...
                        <div className="fixed bottom-14 bg-[#F1C232] py-5 px-4 rounded-t-lg md:w-[30.375rem] w-screen">
                          ...
                          <button
                            className="text-xl text-center bg-[#402e32] text-white w-full py-3 mt-3 rounded-md"
                            onClick={checkouted}
                          >
                            Lanjut Bayar
                          </button>
